...
import numpy as np
import resampy
from model import OSDExecutive
...
def inference(file: UploadFile = File(...)):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        osd_result = MODEL(wav_path)
        return JSONResponse(content=to_jsonable(osd_result))

if __name__ == "__main__":
    uvicorn.run("app:app", host="0.0.0.0", port=8029, reload=True)